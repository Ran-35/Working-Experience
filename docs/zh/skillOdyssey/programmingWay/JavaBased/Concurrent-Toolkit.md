# 并发工具包
Java并发工具包（java.util.concurrent，简称JUC）是Java 5.0引入的一组强大的并发编程工具类，提供了比传统的synchronized和wait()/notify()更高级的并发控制机制。

## 原子类（Atomic Classes）
旨在提供高效的、无锁的线程安全操作。它们用于在多线程环境中以原子操作的方式对变量进行读写，避免了传统的同步（synchronized）机制的开销，同时提供了更高的性能。

### 原子操作
1. 要么完全执行，要么完全不执行，不会被其他线程中断。
2. Java 中的原子类使用底层的硬件指令来实现这些原子操作。
3. 多线程环境下，原子操作可以保证数据一致性，保证了共享变量的修改不被其他线程打断。
...
### 原子类型包装类
提供对基本数据类型的原子操作，基本上是对 int、long、boolean 等数据类型的封装，支持原子加减、设置、获取等操作。

AtomicInteger：对 int 类型的原子操作。  
AtomicLong：对 long 类型的原子操作。  
AtomicBoolean：对 boolean 类型的原子操作。  
AtomicReference< T >：对对象引用（例如 T）的原子操作，使用CAS保证原子性。  
AtomicStampedReference< T >：支持 stamp（版本标记）机制的原子引用，防止ABA问题。  
AtomicMarkableReference< T >：支持布尔标记的原子引用，也可以避免ABA问题。  
```Java
public class Atomic {
    public static void main(String[] args) {
        ...
        // 比较并替换(如果当前值是9，替换为11，成功返回true,失败返回false)
...
public class Atomic {
    public static void main(String[] args) {
        ...
        // 比较并交换(将3处的3改为100，若3处不为3，就false)
...
### 字段更新器
Java中用于执行原子性操作的类，允许对对象的特定字段进行原子更新。
```Java
public class Atomic {
    public static void main(String[] args) {
        ...
        // 创建一个原子更新器
        AtomicReferenceFieldUpdater<Computer,String> updater = AtomicReferenceFieldUpdater.newUpdater(Computer.class, String.class, "cpu");
...
### 累加器
一种特殊的原子变量，主要用于支持并发环境下高效地执行加法操作。

LongAdder 和 DoubleAdder 是专为高并发设计的类，它们提供了比 AtomicLong 更高效的并发累加方式。它们通过 分段（striping） 的方式来减少在高并发下的争用。

LongAdder 用于 long 类型的加法。

DoubleAdder 用于 double 类型的加法。

这些类适用于高并发的场景，尤其是当你需要频繁地对共享变量进行累加操作时。
```Java
public class Atomic {
    public static void main(String[] args) {
...
## 同步器（Synchronizer）
同步器（Synchronizer）是一种用于协调多个线程之间的执行顺序和共享资源访问的机制。同步器通常用于控制线程之间的同步与通信，使得它们能够在并发环境中安全地共享数据或执行任务。

### CountDownLatch（计数器倒计时）
· CountDownLatch 是一种同步工具，用于将多个线程的执行控制在某个点，直到所有线程都完成某些任务才继续执行后续操作。  
· CountDownLatch 的构造函数需要一个整数参数，表示需要等待的线程数量。  
· 当调用 countDown() 方法时，计数器减 1，当计数器为 0 时，等待的线程继续执行。
...
### CyclicBarrier（循环栅栏）
CyclicBarrier 用于使一组线程在某个公共屏障点上等待，直到所有线程都到达该屏障点。与 CountDownLatch 不同的是，CyclicBarrier 允许线程在同一屏障点循环多次，适用于反复使用的场景。
